...
## Web App
A standalone page lives in `webapp/index.html`. Open it in a browser to try the beatmaker without installing the extension. The page loads a sample video, lets you choose your own clip, and uses small JavaScript stubs to mimic Chrome extension APIs so the full interface can run in a regular tab. The web app also recreates the YouTube player DOM (`.html5-video-player`, `.ytp-chrome-bottom`, `.ytp-right-controls`, and the progress bar) so `content.js` attaches its UI in the same way as on YouTube, preserving features and low-latency cue playback on phones and tablets. Custom play/pause controls, a time display, progress bar seeking, and familiar keyboard shortcuts mirror the YouTube player for touch and desktop use.
## New in 1.6.2
